      ...
      <div className="p-8 max-w-4xl">
        ...
        <form onSubmit={handleStart} className="bg-court-900 border border-court-700 rounded-xl p-6 space-y-4 mb-8">
              ...
              >
                {rateCard.map((r) => (
                  <option key={r.id} value={r.id}>
                    {r.unit_minutes} min — AED {r.price}
                  </option>
                ))}
              </select>
            </div>
          ...
          <p className="text-[11px] text-line-dim">
            Amount pre-fills from the rate card — edit it if they paid a different amount (half session, offer, etc).
          </p>
          ...
          <button
            type="submit"
            disabled={saving || rateCard.length === 0}
          ...
          {rateCard.length === 0 && (
            <p className="text-xs text-danger">
              No billiards rates found — run migration_2.sql in Supabase to add the rate card.
            </p>
          )}
        </form>
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <p className="text-sm text-line-dim">
            Collected: <span className="text-chalk font-mono">AED {todayTotal.toFixed(0)}</span>
          </p>
        </div>
              ...
              {rentals.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-5 py-8 text-center text-line-dim text-sm">
                    No table rentals logged today yet.
                  </td>
                </tr>
              )}
            </tbody>
